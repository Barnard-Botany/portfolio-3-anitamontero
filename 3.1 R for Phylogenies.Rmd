...
tr_primates
Phylogeny is rooted and has no branch lengths

str("tr_primates")
It is a character object 

and

str(tr_primates)
Tells the value of the edge and the Nnode, tip lables, and that it is a chr phylo and cladewise object, and that it is a list of 3

attributes(tr_primates)
Gives the options for names (edge, Nnode, and tip.lable), class (phylo), and order (cladewise)

names(tr_primates) 
Gives options for names alone (edge, Nnode, and tip.lable)
...
```{r}
newick_str2 = "((orca, humpback), (manatee, dolphin));"
tr_water_mammals = read.tree(text=newick_str2)
plot(tr_water_mammals)
```
...
This is technical but basic jargon. What is complex and rich is the biological and evolutionary meaning in this framework, important for understanding conservation, ethnobotany, ecology, conservation, or crop breeding, and evolutionary processes such as migration, hybridization, speciation or extinction. 
...
Can tell this is botanically incorrect. Cabbage and kale are the same plant and corn and wheat are more closely related!
...
Yes, phylogeny below is better because kale and cabbage are together but corn and wheat should be more closely related because corn is a monocot
...
```{r}
newick_str = "((Cabbage, Kate), (Corn, Wheat));"
tr_plants = read.tree(text=newick_str)
plot(tr_plants)
```
...
In the erroneous tree, radish, kale, and cabage are depicted as being equally related when in reality kale and cabbage are more related to each other than to radish, although they are all part of the mustard family. Additonally, sequoia, wheat, and corn are depicted as equally realted when in reality corn and wheat are more related to each other becasue they are both monocots and sequoia is the outgroup becasue it is a gymnosperm.In the correct tree, kale and cabbage are depicted as monophyletic with radish sharing a more distant common ancestor. Wheat and Corn are monophyletic and share a distant common ancestor with the kale, cabbage, radish group and sequoia is the outgroup
...
Trees have the same number of edges, but the correct tree has more tips (12 vs. 9) and Nnodes (7 vs. 4) than the erroneous tree. 
...
```{r}
newick_str = "((((Cabbage, Kale), (radish)), (Corn, Wheat)), (sequoia));"
tr_plants3 = read.tree(text=newick_str)
plot(tr_plants3)
```

```{r}
newick_str = "((Cabbage, Kale, (radish)), (Corn, Wheat, sequoia));"
tr_plants4 = read.tree(text=newick_str)
plot(tr_plants4)
```
...
What was added to the tree and the plot of the tree? Branch lengths!
...
```{r}
newick_str = "(((Humans:12.5, Chimps:12.5):1.0, Gorillas:14.0):1.0, (Pongo abelii:3.5, Pongo tapanuliensis:3.5):14.5):1.0;"
tr_primate = read.tree(text=newick_str)
plot(tr_primate)
```
...
```{r}
plot(tr_plants, type="phylogram", direction="rightwards")
plot(tr_plants, type="phylogram", direction="leftwards")
plot(tr_plants, type="phylogram", direction="upwards")
plot(tr_plants, type="phylogram", direction="downwards")
```

```{r}
plot(tr_plants, type="cladogram")
plot(tr_plants, type="fan")
plot(tr_plants, type="unrooted")
plot(tr_plants, type="radial")
```
...
```{r}
layout(matrix(1:4, 2, 2))
plot(tr_plants, type="unrooted", edge.width=5)
plot(tr_plants, type="unrooted", edge.width=5, edge.color="blue")
plot(tr_plants, type="unrooted", edge.width=5, edge.color="blue", lab4ut="horizontal")
plot(tr_plants, type="unrooted", edge.width=5, edge.color="blue", lab4ut="axial")
```
...
trnew_plants <- rotate(tr_plants, c("Cabbage", "Kale"))
plot(trnew_plants)
# compare the results:
par(mfrow=c(1,2)) # split graphical device
plot(tr_plants) # plot old tre
...
This is the phylogeny I used above but I've included some new display techniques below. 

```{r}
plot(tr_plants, type="fan")
plot(tr_plants, type="unrooted")
plot(tr_plants, type="unrooted", edge.width=5, edge.color="red", lab4ut="horizontal")
plot(ladderize(tr_plants), main = "right-ladderized")
plot(ladderize(tr_plants, FALSE), main = "left-ladderized")
```
...
```{r}
newick_fn = "tr_plants"
write.tree(tr_plants, file="tr_plants")
```
...
urlA<-"http://datadryad.org/bitstream/handle/10255/dryad.8791/final_tree.tre?sequence=1"
Abies<-phylomatic(taxa=spp1, treeuri=urlA)
plot(Abies, type="unrooted")

spp2 <- read.table("slashpath.txt")
spp2
tree_phylomatic <- phylomatic(taxa = spp2)
#We have dropped the Get =POST command.
plot(tree_phylomatic, type ="unrooted", no.margin = TRUE)
...
EG<-c("Abies","Ailanthus","Aspicllum","Atragene","Bangia","Begonia","Berberis","Betula","Blasia","Blechnum","Caulerpa","Ceratophyllum","Chenopodium","Clematis","Cocculus","Corallorrhiza","Corchorus","Cucurbita","Dahlia","Dioscorea","Dracaena","Equisetum","Euonymus","Euphorbia","Fagus","Ficus","Fontinalis","Fossombronia","Fritillaria","Fucus","Gleditschia","Gleocapsa","Haplomitrium","Hedera","Helianthus","Heterocentron","Hippuris","Hypoderma","Isoetes","Linum","Liquidambar","Marchantia","Momordica","Monopodium","Mucor","Nerium","Nitella","Osmunda","Pandanus","Papaver","Pelargonium","Phlomis","Phytolacca","Pinus","Platanus","Polytrichum","Populus","Potamogeton","Punctum","Quercus","Ricinus","Rubus","Salix","Sambucus","Sclerotium","Selaginella","Sempervivum","Sponia","Stypocaulon","Sympodium","Taxodium","Tecoma","Tradescantia","Urtica","Viola","Vitis","Wisteria","Yucca")
EG
tree_phylomatic_EG1 <- phylomatic(taxa = EG, get = 'POST')
plot(tree_phylomatic_EG1, no.margin = TRUE, type = "unrooted")
newick_fn4 = "tree_phylomatic_EG1"
write.tree(tree_phylomatic_EG1, file=newick_fn4)

tree_phylomatic_EG2 <- phylomatic(taxa = EG, storedtree ="zanne2014", get = 'POST')
plot(tree_phylomatic_EG2, type = "unrooted", no.margin = TRUE)
newick_fn5 = "tree_phylomatic_EG2"
write.tree(tree_phylomatic_EG2, file=newick_fn5)
```
...
The unrooted trees (perhaps obviously) do not identify an ancestral root, as compared to the cladograms and phlyograms which do. As a result, the tree depicts species relatedness but not their historical ancestry. This indicates that there is much more information in the storedtree(s) than is necessarily depicted in each tree.
...
The output says that 56 taxa were matched. Of these, some were misspelled and some were non-plant genera, but the vast majority were ferns, aquatic plants, mosses, and liverworts. This indicates that the evolutionary relationships of non-terrestrial, non-vascular, and non-seeded plants are not well known, perhaps because they have been less thoroughly studied throughout the history of botany. Within the group of unmatched taxa, I was surprised to see two gymnosperm genera, pinaceae and abies, and an angiosperm genus, vitis. Despite their exclusion, I suspect that there is robust phylogenetic data on these genera and that they should have been included.  
...
```{r}
welikia<-c("Acalypha virginica", "Acer rubrum", "Dichanthelium depauperatum", "Erigeron strigosus", "Parthenocissus quinquefolia", "Prunus serotina", "Smilax rotundifolia", "Viburnum prunifolium", "Castanea dentata", "Eurybia divaricata", "Quercus rubra", "Rhus copallinum", "Viburnum acerifolium", "Muhlenbergia schreberi", "Boechera laevigata", "Aralia nudicaulis", "Asclepias tuberosa", "Aureolaria flava", "Carex umbellata", "Cuscuta gronovii", "Desmodium nudiflorum", "Dryopteris marginalis", "Festuca rubra", "Fragaria virginiana", "Galium circaezans", "Galium triflorum", "Gaylussacia baccata", "Geranium maculatum", "Geum virginianum", "Hamamelis virginiana", "Juniperus virginiana", "Kalmia latifolia", "Lespedeza procumbens", "Linum virginianum", "Lonicera sempervirens", "Maianthemum racemosum", "Mitchella repens", "Muhlenbergia sobolifera", "Oenothera biennis", "Panicum virgatum", "Phegopteris hexagonoptera", "Phryma leptostachya", "Physalis heterophylla", "Pinus rigida", "Polygonum scandens", "Polystichum acrostichoides", "Potentilla norvegica", "Quercus velutina", "Rosa carolina", "Silene caroliniana")
welikia
tree_welikia <- phylomatic(taxa = welikia, get = 'POST')
plot(tree_welikia, no.margin = TRUE, type = "phylogram")
pdf_fn = "tree_welikia.pdf"
pdf(file=pdf_fn)
plot(tree_welikia, type="phylogram")
dev.off()
newick_fn = "tree_welikia"
write.tree(tree_welikia, file="tree_welikia")
```
```{r}
welikia <- read.table("welikia_list.txt")
welikia
tree_welikia <- phylomatic(taxa = welikia)
plot(tree_welikia, no.margin = TRUE)
```

```{r}
plantomatic<-c("Aegopodium podagraria", "Agrimonia striata", "Alternanthera caracasana", "Amelanchier arborea", "Anemone cylindrica", "Apocynum androsaemifolium", "Arthraxon hispidus", "Aureolaria virginica", "Bidens laevis", "Brassica nigra", "Bryum dichotomum", "Caltha palustris", "Campylopus tallulensis", "Carex backii", "Carya glabra", "Chamaecrista nictitans", "Chloris verticillata", "Clematis virginiana", "Conioselineum chinense", "Crataegus chrysocarpa", "Cyperus erythrorhizos", "Desmodium obtusum", "Dichondra carolinensis", "Dionaea muscipula", "Drepanocladus longifolium", "Echinochloa muricata", "Elymus canadensis", "Erysimum inconspicuum", "Fimbristylis puberula", "Funaria hygrometrica", "Geranium carolinianum", "Gymnocarpium dryopteris", "Herzogiella turfacea", "Hygroamblystegium fluviatile", "Hypnum plicatulum", "Iris virginica", "Juncus secundus", "Lechea pulchella", "Lepidium virginicum", "Lipocarpha micrantha", "Ludwigia alternifolia", "Lysimachia ciliata", "Medeola virginiana", "Mitchella  repens", "Muhlenbergia uniflora", "Nuphar lutea", "Ornithogalum umbellatum", "Pachysandra terminalis", "Paspalum conjugatum", "Perilla frutescens")
plantomatic
tree_plantomatic <- phylomatic(taxa = plantomatic, get = 'POST')
plot(tree_plantomatic, no.margin = TRUE, type = "phylogram")
pdf_fn = "tree_plantomatic.pdf"
pdf(file=pdf_fn)
plot(tree_plantomatic, type="phylogram")
dev.off()
newick_fn = "tree_plantomatic"
write.tree(tree_plantomatic, file="tree_plantomatic")
```

```{r}
plantomatic <- read.table("PlantOMatic_List.txt")
plantomatic
tree_plantomatic <- phylomatic(taxa = plantomatic)
plot(tree_plantomatic, no.margin = TRUE)
```
